...
describe('FSM', () => {
  ...
  it('should process promise', async () => {
    ...
    const promiseState = machine.state('promise', {
    ...
    }, (data, err) => anotherPromiseState.build({
      abc: data.val,
      err
    }))
  ...
  it('should process event', async () => {
    ...
    const eventState = machine.state('event', {
      schema: z.object({ val: z.number() }),
    }).initWithEvents([
  ...
  it('should process final', async () => {
    ...
    const finalState = machine.state('final', {
      schema: z.object({ val: z.number() }),
    }).initAsFinal()
  ...
  it('should process parent', async () => {
    ...
    const parentState = machine.state('parent', {
      schema: z.object({ val: z.number() }),
    }).initAsParent()

    const child1 = parentState.subState('child1', {
    ...
    const child2 = parentState.subState('child2', {
  ...
  it('should process simple flow', async () => {
    ...
    const promiseState = machine.state('promise', {
      ...
      return res
    }, (err) => failedState.build({
      err
    }))

    const failedState = machine.state('failed', {
      schema: z.object({
        err: z.any()
      }),
    }).initAsFinal()
    ...
    const divided = parentState.subState('divided', {
      schema: z.object({
        numerator: z.number()
      }),
    }).initWithEvents([
  ...
  it('should contain info', () => {
    ...
    parent.subState('child1', {
      schema: z.void(),
    }).initAsFinal()

    parent.subState('child2', {
      schema: z.object({
        bar: z.string()
      }),
      info: {
    ...
    const parent2 = parent.subState('parent2', {
      schema: z.object({
        baz: z.string()
      }),
      info: {
