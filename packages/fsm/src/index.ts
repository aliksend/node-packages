...
export class StateMachine<Info extends object | void = void> {
   ...
   */
  state<Schema extends z.ZodType>(
    name: string,
    def: StateDefinition<Schema, Info>,
  ): StateDeclaration<void, Schema, Info> {
 ...
 */
type InfoDefinition<Schema extends z.ZodType, Info extends object | void> = Info extends void
  ? void
  : {
...
class StateDeclaration<ParentSchema extends z.ZodType | void, Schema extends z.ZodType, Info extends object | void> {
  constructor(
    ...
    private readonly def: StateDefinition<MakeParentSchema<ParentSchema, Schema>, Info>,
    private readonly newStateBuilderCallback: (b: StateBuilder<any, any, Info>) => void,
    private readonly parentStateBuilder: ParentSchema extends z.ZodType
      ? ParentStateBuilder<ParentSchema, Info>
      : undefined,
  ) {}
   ...
   */
  initWithEvents(
    events: Array<EventDefinitionWithHandler<any, MakeParentSchema<ParentSchema, Schema>, Info>>,
  ): StateBuilder<ParentSchema, Schema, Info> {
   ...
   */
  initWithPromise(
    promiseCb: (data: z.output<MakeParentSchema<ParentSchema, Schema>>) => Promise<State<Info>>,
    catchCb?: (data: z.output<MakeParentSchema<ParentSchema, Schema>>, err: Error) => State<Info>,
  ): StateBuilder<ParentSchema, Schema, Info> {
    const stateBuilder = new StateBuilder(
      this.name,
      this.def,
      { type: "PROMISE", promiseCb, catchCb },
      this.parentStateBuilder,
    );
    this.newStateBuilderCallback(stateBuilder);
    return stateBuilder;
  }
  ...
  initAsParent(): ParentStateBuilder<MakeParentSchema<ParentSchema, Schema>, Info> {
    const stateBuilder = new ParentStateBuilder(
      this.name,
      this.def,
      this.newStateBuilderCallback,
      this.parentStateBuilder,
    );
 ...
 */
type MakeParentSchema<ParentSchema extends z.ZodType | void, Schema extends z.ZodType> = ParentSchema extends z.ZodType
  ? z.ZodObject<{
      $value: Schema;
      $parent: ParentSchema;
    }>
...
class ParentStateBuilder<ParentSchema extends z.ZodType, Info extends object | void> {
  constructor(
    private readonly name: string,
    private readonly def: StateDefinition<ParentSchema, Info>,
    private readonly newStateBuilderCallback: (b: StateBuilder<any, any, Info>) => void,
    private readonly parentStateBuilder: ParentStateBuilder<ParentSchema, Info> | undefined,
  ) {}

  /**
   * Создать состояние-потомок, наследующее схему данных (в параметре $parent) и info (может быть переопределено)
   */
  subState<Schema extends z.ZodType>(
    name: string,
    def: {
      schema: Schema;
    ...
    return new StateDeclaration<ParentSchema, Schema, Info>(
      name,
      {
        schema: z.object({
          $value: def.schema,
...
class ParentState<Info extends object | void> {
  constructor(
    ...
    protected readonly infoDefinition: InfoDefinition<any, Info>,
    protected readonly parentState: ParentState<Info> | undefined,
  ) {}
...
class StateBuilder<ParentSchema extends z.ZodType | void, Schema extends z.ZodType, Info extends object | void> {
  constructor(
    ...
    private readonly def: StateDefinition<MakeParentSchema<ParentSchema, Schema>, Info>,
    private readonly transitions: Transitions<MakeParentSchema<ParentSchema, Schema>, Info>,
    private readonly parentStateBuilder: ParentSchema extends z.ZodType
      ? ParentStateBuilder<ParentSchema, Info>
      : undefined,
  ) {}
  ...
  build(data: z.input<MakeParentSchema<ParentSchema, Schema>>, skipInit: boolean = false): State<Info> {
    const parsed = this.def.schema.parse(data);
    const infoDefinition: InfoDefinition<MakeParentSchema<ParentSchema, Schema>, Info> = "info" in this.def
      ? (this.def.info as any)
      : undefined;
...
export class State<Info extends object | void> extends ParentState<Info> {
  constructor(
    readonly name: string,
    data: unknown,
    private readonly transitions: Transitions<any, Info>,
    infoDefinition: InfoDefinition<any, Info>,
    parent: ParentState<Info> | undefined,
  ) {
...
export class EventDefinition<ArgSchema extends z.ZodType, Info extends object | void> {
   ...
   */
  handler<Schema extends z.ZodType>(
    cb: (data: z.output<Schema>, arg: z.output<ArgSchema>) => State<Info> | void,
  ): EventDefinitionWithHandler<ArgSchema, Schema, Info> {
 ...
 */
export class EventDefinitionWithHandler<
  ArgSchema extends z.ZodType,
  Schema extends z.ZodType,
  Info extends object | void,
> {
  constructor(
    readonly name: string,
    private readonly argSchema: ArgSchema,
    private readonly cb: (data: z.output<Schema>, arg: z.output<ArgSchema>) => State<Info> | void,
  ) {}
